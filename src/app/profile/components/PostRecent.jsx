...
export default function PostRecent({posts}) {
  return (
    <main id="main" className="content">
        {posts.map((post) => (<PostRecentCard post={post} key={post.title} />))}
...
function PostRecentCard({post}) {
  return (
    <div className="featured-posts outer">
        <article className="post tag-javascript featured post-featured inner">
            <header className="post-header">
                <div className="post-meta">
                    <span className="post-featured-badge">recent</span> # <time className="published" dateTime={post.datetime}>{post.datetime}</time>
                </div>
